...
def acceso(usuario="sebastian.mgonzalez@hotmail.com",contraseña="############"):
...
def refresco(usuario="sebastian.mgonzalez@hotmail.com",contraseña="############"):
    return json.loads(requests.post("https://api.invertironline.com/token",data={
        "username":usuario,
        "password":contraseña,
        "grant_type":"password"        
...
while True and datetime.now().time() >= hora_min and datetime.now().time() <= hora_max:
    ...
    if ordenes_pendientes(activo_est)[0] == True:                  # Si sigue pendiente a los 3 minutos, cancelamos y mandamos otra
    ...
    #df.to_excel(r'C:\Users\user\Desktop\SMG\Trading Algorítmico\GGALdf.xlsx', index = False, header=True)
